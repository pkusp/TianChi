...
###
###  gbdt0523_v2.py  增加特征 前一周同时间段，增加MAPE测评，n_estimators 为 6 时效果较好
###
###  xgboost0524.py 使用gbdt整理好的数据，尝试带入xgboost进行训练，使用github-xgboost-demo-multiclass classification的模板
###  xgboost0524.py 参数一点没调，比较粗糙，错误没有校验
###
##############################################
import numpy as np
...
def xgb_morning(fileName):
	...
	for i in range(6): 				### 六个时间窗口分别训练并验证
		...
		train_y_final=train_76[:,j]   #trian[]已经降为76行
		####################################################################
		train_x=np.mat(train_x_final)    #'''  train_x here '''              ##转化为原名，方便后续函数调用
		train_y=train_y_final            #'''  train_y  here '''
		###############################################################################   TRAIN data 处理结束   ########################
		...
		test_x=np.mat(test_x_final)         #'''  待预测数据 test_x  '''
		################################################################################   TEST data 处理结束   ##########################
	...
	###################################################### to csv
	resultDF=DataFrame(resultList)         ## mat to DF 
	resultDF.to_csv(fileName+'_moring_predict_0524_v1.csv')
	return sumScore,mapeValue

def xgb_afternoon(fileName):
	...
	###################################################### to csv
	resultDF=DataFrame(resultList)         ## mat to DF 
	resultDF.to_csv(fileName+'_afternoon_predict_0524_v1.csv')
	return sumScore,mapeValue


'''
xgb_boost=xgb.XGBRegressor(seed=1440)
xgb_boost.fit(train_data[..],train_label)
result=xgb_boost.predict(test_data[])

score+=evaluate(ori_label,result)

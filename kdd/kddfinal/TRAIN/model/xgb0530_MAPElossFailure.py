...
###
###  xgb0530 修改loss def mapeobj,evalerror                                               2017.05.30  失败 ！！！ 没时间调了
##############################################
import numpy as np
...
#####################################################################################  XGB CV customize
'''
print ('running cross validation, with MAPE loss function')
def mapeobj(preds, dtrain):
    labels = dtrain.get_label()
    grad = ( 4/labels**4 )*( (preds-labels)**3 )
    hess = ( 12/labels**4 )*( (preds-labels)**2 )
    return grad, hess

def evalerror(preds, dtrain):
    labels = dtrain.get_label()
    return 'error', ( ((labels-preds)/labels)**4 ).mean    #float(sum(labels != (preds > 0.0))) / len(labels)

# train with customized objective
xgb.cv(param, dtrain, num_round, nfold = 5, seed = 0,
       obj = mapeobj, feval=evalerror)
'''
#####################################################################################  XGB CV customize
...
def morning_xgb(fileName):     #return sumScore
	...
	for i in range(6): 				### 六个时间窗口分别训练并验证
		...
		def mapeobj(preds, dtrain):
			...
			#labels= 1.0 / (1.0 + np.exp(-labels))
			#grad = ( 4/labels**4 )*( (preds-labels)**3 )
			#hess = ( 12/labels**4 )*( (preds-labels)**2 )

			grad=(2/labels**2)*(preds-labels)*100000000
			hess=2/labels**2*10000000
			#labels = dtrain.get_label()
			#preds = 1.0 / (1.0 + np.exp(-preds))
			#grad = preds - labels
		...
		def evalerror(preds, dtrain):
			...
			#preds= 1.0 / (1.0 + np.exp(-preds))
			#labels= 1.0 / (1.0 + np.exp(-labels))
			return 'MAPE', ( 100000000*((labels-preds)/labels)**2 ).mean()    #float(sum(labels != (preds > 0.0))) / len(labels)

		# train with customized objective	
		#xgbCV_03=xgb.cv(param, train_final, num_round, nfold = 5, seed = 1000,obj = mapeobj, feval=evalerror)
		
		##################  CV   ##########################
		#xgb_cv=xgb.train(param, train_final, num_round, obj = mapeobj, feval=evalerror,maximize=False)
		...
		train_cv=xgb.DMatrix(train_X_cv,label=train_Y_cv)   ## 数据格式转换
		xgb_cv=xgb.train(param, train_cv, num_round, obj = mapeobj, feval=evalerror)#,maximize=False)
		#xgb.train(train_X_cv,train_Y_cv)
		test_X_cv=xgb.DMatrix(test_X_cv)
		pred=xgb_cv.predict(test_X_cv)
		...
		xgb_final=xgb.train(param, train_final, num_round, obj = mapeobj, feval=evalerror)#,maximize=False)#,watchlist)

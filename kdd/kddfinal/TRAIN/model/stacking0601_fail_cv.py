	...
	for i in range(6): 				### 六个时间窗口分别训练并验证
		...
		def xgb_01(train_x,train_y,test_x):
			...
			def xgb_validation(x_name,label_name):   
				...
				train_X_cv, test_X_cv, train_Y_cv, test_Y_cv = cross_validation.train_test_split(x_name, label_name, test_size=0.3, random_state=0)  # validation			
		...
		#######  test
		pred11,yhat11=xgb_01(evl_x,evl_y,test_x)
		pred12,yhat21=gbdt_02(evl_x,evl_y,test_x)
		pred13,yhat31=svr_03(evl_x,evl_y,test_x)
		pred14,yhat41=knr_04(evl_x,evl_y,test_x)
		pred15,yhat51=linear_05(evl_x,evl_y,test_x)
		pred16,yhat61=rfr_06(evl_x,evl_y,test_x)
		pred17,yhat71=dnn_07(evl_x,evl_y,test_x)
		...
		yhat11=np.array(yhat1)#;pred1=np.array(pred1)  ###  to ndarray
		yhat21=np.array(yhat2)#;pred2=np.array(pred2)
		yhat31=np.array(yhat3)#;pred3=np.array(pred3)
		yhat41=np.array(yhat4)#;pred4=np.array(pred4)
		yhat51=np.array(yhat5)#;pred5=np.array(pred5)
		yhat61=np.array(yhat6)#;pred6=np.array(pred6)
		yhat71=np.array(yhat7)#;pred7=np.array(pred7)

		yhat11=np.mat(yhat1).T;# pred1=np.mat(pred1).T
		yhat21=np.mat(yhat2).T;# pred2=np.mat(pred2).T
		yhat31=np.mat(yhat3).T;# pred3=np.mat(pred3).T
		yhat41=np.mat(yhat4).T;# pred4=np.mat(pred4).T
		yhat51=np.mat(yhat5).T;# pred5=np.mat(pred5).T
		yhat61=np.mat(yhat6).T;# pred6=np.mat(pred6).T
		yhat71=np.mat(yhat7).T;# pred7=np.mat(pred7).T
		...
		stacking_evl=np.hstack((yhat11,yhat21,yhat31,yhat41,yhat51,yhat61,yhat71))  ###  data
		...
		def gbdt_stacking(train_x,train_y,test_x):
			...
			pre_evl=gbdt_stacking.predict(evl_x)
			MAPE(evl_y,pre_evl)
			print('\n\nthe fucking MAPE is :\n')
			print(MAPE)
		...
		#### test
		pred_evl,yhattest=gbdt_stacking(stacking_evl, evl_y,test)

		mappe=MAPE(evl_y,yhattest)
		print('\n\n\n the fucking mapeErr')
		print(mappe)
...
def afternoon_stacking(fileName):     #return sumScore
	...
	for i in range(6): 				### 六个时间窗口分别训练并验证
		...
		def data_pre_PM():		
			...
			return train_x,train_y,test_x
		############################# pre data end #######
		train_x,train_y,test_x = data_pre_PM()		
		...
		def rfr_06(train_x,train_y,test_x):
			...
			#resultArr.append(pred_rfr)     ##   导出结果
			return pred_rfr
		#########################################################  stacking function over #################
		print('\n')
		s1=svr_03(train_x,train_y,test_x)
		s2=knr_04(train_x,train_y,test_x)
		s3=linear_05(train_x,train_y,test_x)
		s4=rfr_06(train_x,train_y,test_x)

		pred_final=0.4*s1+0.25*s2+0.1*s3+0.25*s4
		resultArr.append(pred_final)     ##   stacking结果
		###################################################################  stacking over  ################
	resultArr = np.array(resultArr)  ###### 为了方便导出csv
	resultList=list(resultArr[:,0])+list(resultArr[:,1])+list(resultArr[:,2])+list(resultArr[:,3])+list(resultArr[:,4])+list(resultArr[:,5])+list(resultArr[:,6])
	...
	###################################################### to csv
	resultDF=DataFrame(resultList)         ## mat to DF 
	resultDF.to_csv(fileName+'_PM_predict_stacking_v2.csv')
	return 0#xgbMape
...
'''
		...
		#resultArr.append(pred_final)     ##   stacking结果
		#########################################################  stacking end  #################
		resultArr1.append(pred1)
		resultArr2.append(pred2)
		resultArr3.append(pred3)
		resultArr4.append(pred4)
		resultArr5.append(pred5)
		resultArr6.append(pred6)
		resultArr7.append(pred7)
		resultArr8.append(pred8)
	#####################################################  real stacking here !!!  #####################

	#resultMat=resultMat.T           # .T
	#resultArr = np.array(resultArr)  ###### 为了方便导出csv
	resultArr1 = np.array(resultArr1)
	resultArr2 = np.array(resultArr2)
	resultArr3 = np.array(resultArr3)
	resultArr4 = np.array(resultArr4)
	resultArr5 = np.array(resultArr5)
	resultArr6 = np.array(resultArr6)
	resultArr7 = np.array(resultArr7)
	resultArr8 = np.array(resultArr8)

	#resultList=list(resultArr[:,0])+list(resultArr[:,1])+list(resultArr[:,2])+list(resultArr[:,3])+list(resultArr[:,4])+list(resultArr[:,5])+list(resultArr[:,6])
	resultList1=list(resultArr1[:,0])+list(resultArr1[:,1])+list(resultArr1[:,2])+list(resultArr1[:,3])+list(resultArr1[:,4])+list(resultArr1[:,5])+list(resultArr1[:,6])
	resultList2=list(resultArr2[:,0])+list(resultArr2[:,1])+list(resultArr2[:,2])+list(resultArr2[:,3])+list(resultArr2[:,4])+list(resultArr2[:,5])+list(resultArr2[:,6])
	resultList3=list(resultArr3[:,0])+list(resultArr3[:,1])+list(resultArr3[:,2])+list(resultArr3[:,3])+list(resultArr3[:,4])+list(resultArr3[:,5])+list(resultArr3[:,6])
	resultList4=list(resultArr4[:,0])+list(resultArr4[:,1])+list(resultArr4[:,2])+list(resultArr4[:,3])+list(resultArr4[:,4])+list(resultArr4[:,5])+list(resultArr4[:,6])
	resultList5=list(resultArr5[:,0])+list(resultArr5[:,1])+list(resultArr5[:,2])+list(resultArr5[:,3])+list(resultArr5[:,4])+list(resultArr5[:,5])+list(resultArr5[:,6])
	resultList6=list(resultArr6[:,0])+list(resultArr6[:,1])+list(resultArr6[:,2])+list(resultArr6[:,3])+list(resultArr6[:,4])+list(resultArr6[:,5])+list(resultArr6[:,6])
	resultList7=list(resultArr7[:,0])+list(resultArr7[:,1])+list(resultArr7[:,2])+list(resultArr7[:,3])+list(resultArr7[:,4])+list(resultArr7[:,5])+list(resultArr7[:,6])
	resultList8=list(resultArr8[:,0])+list(resultArr8[:,1])+list(resultArr8[:,2])+list(resultArr8[:,3])+list(resultArr8[:,4])+list(resultArr8[:,5])+list(resultArr8[:,6])

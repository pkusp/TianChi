...
def loadData():
	...
	for fileName in listName:
		...
		fullName=fileName+'_AM_predict_stacking_yhat+x+holiday+ss.csv'
		...
		#print(type(dataList))
		sample=pd.read_csv('submission_phase2_v2_travelTime.csv',index_col=0)
		#print(sample)
		#print(sample)
		#print(type(sample))
		...
		j=j+42
		k=k+42
		sample.to_csv('submission_phase2_v2_travelTime.csv')
	...
	for fileName in listName:
		...
		fullName=fileName+'_PM_predict_stacking_yhat+x+holiday+ss.csv'
		data=pd.read_csv(fullName)
		data=data.drop('Unnamed: 0',axis=1) #去掉奇怪的一列
		dataSeries=data['0']
		dataList=list(dataSeries)

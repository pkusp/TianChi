...
def data_pre(fileName):
	...
	train_x=train_v[:,:49]      # 用前面50个数据
	train_y=train_v[:,49:50]	# 预测第 50 个数据
	print('data pre end...')
...
train_x,train_y,train_arr=data_pre('round_mat_T_sales.csv') # tain_x train_y here !!!

pred_list=[]
for i in range(81,92):

	#print('predict number',i,' day')
	test_x=train_arr[:,i-49:i]                   #  test here !!! in  for()
	...
	yhat1=np.array(yhat1);pred11=np.array(pred1)  ###  to ndarray
	...
	yhat1=np.mat(yhat1).T; pred1=np.mat(pred11).T
	...
	############### stacking #######################################
	stacking_train=np.hstack((yhat1,yhat2,yhat6))#,yhat7))  ###  data
	stacking_label=train_y
	stacking_test=np.hstack((pred1,pred2,pred6))#,pred7))
	...
	MAPE_XGB=MAPE(train_arr[:,i],pred1)
...
#print(pred_df)
pred_df.to_csv('pred_81_91_each_round_stacking_50features.csv')

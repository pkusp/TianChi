...
sales=sales[:3500]		# 取2000个sale作为最终预测  ## 权宜之计，计算速度太慢 20170824
buyers=buyers[:3500]
...
def link_3d_pre(fileName):
	...
	data_arr_t=data_arr_t[2:3502,1:3501] # 去掉名称行列，取前2000
	...
	for cnt in range(7):
		...
		cnt_list=[[],[],[]] ############ 最终3行格式 sales buyer round
		...
		for i in sales[cnt*500:(cnt+1)*500]:
			...
			cnt_arr=np.hstack((cnt_arr,temp_arr)) # 这个要执行上千次，太蠢了！

			count=count+1 # 每次append下一行对所有buyer的销售额

		final_arr=np.hstack((final_arr,cnt_arr))
		print('finish number:',cnt)
	#   return final_arr:
...
final_df.to_csv('link_3d_pre_3500.csv')
